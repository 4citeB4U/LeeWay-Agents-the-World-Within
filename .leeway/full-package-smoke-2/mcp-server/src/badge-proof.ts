...
interface RegistryCategoryEntry {
  name: string;
  displayName: string;
  skillCount: number;
}
...
export async function createLeewaySkillsProof(
  options: CreateProofOptions = {},
): Promise<LeewaySkillsProof> {
  ...
  const badgeSourcePath = path.resolve(
    options.badgeSourcePath ?? DEFAULT_BADGE_SOURCE_PATH,
  );

  const [registryText, badgeBuffer, installedStats, packageVersion] =
    await Promise.all([
  ...
  const registry = JSON.parse(registryText) as SkillsRegistry;
  ...
  const syncStatus =
    installedStats.installedSkills === activeSkills &&
...
export async function installLeewaySkillsBadgeBundle(
  options: InstallBadgeBundleOptions,
): Promise<BadgeBundleInstallResult> {
  ...
  await fs.copyFile(
    path.resolve(options.badgeSourcePath ?? DEFAULT_BADGE_SOURCE_PATH),
    badgeTargetPath,
  );
...
async function scanInstalledSkills(skillsRoot: string): Promise<InstalledSkillStats> {
...
function createBadgeRuntimeScript(): string {
