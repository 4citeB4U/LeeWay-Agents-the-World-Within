...
5WH:
WHAT = Root CLI for the npm-distributed Leeway Skills application bundle
...
function showHelp() {
  console.log("Leeway Skills SDK");
  console.log("");
  console.log("Usage:");
  console.log("  leeway-skills <command> [options]");
  console.log("");
  ...
  console.log("  leeway-skills info");
  console.log("  leeway-skills paths --json");
  console.log("  leeway-skills extract ./leeway-skills-app");
  console.log('  leeway-skills badge ./public "My App" mcp-server');
  console.log("  leeway-skills mcp");
  console.log("  leeway-skills standards doctor");
}
...
async function runExtract() {
  const targetDir = args[1] ?? "leeway-skills-app";
  const overwrite = args.includes("--overwrite");
  const result = await extractLeewayApplication({ targetDir, overwrite });
  console.log("[Leeway Skills SDK] Full application extracted");
...
main().catch((error) => {
  console.error("[Leeway Skills SDK] Fatal error:", error);
  process.exit(1);
});
